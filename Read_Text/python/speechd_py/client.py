...
class SSIPClient(object):
    ...
    def __init__(
        ...
        if _address:
            connection_args.update(
                self._connection_arguments_from_address(_address))
    ...
    def _initialize_connection(self, user, name, component):
        ...
        for event in (CallbackType.INDEX_MARK, CallbackType.BEGIN,
                      CallbackType.END, CallbackType.CANCEL, CallbackType.PAUSE,
                      CallbackType.RESUME):
    ...
    def _connection_arguments_from_address(self, address):
        ...
        if _method == CommunicationMethod.UNIX_SOCKET:
            ...
            try:
                ...
                raise SSIPCommunicationError(
                    "Third parameter of inet_socket address must be a port number"
                )
            except IndexError:
                pass  # The additional parameters was not set, let's stay with defaults
        else:
            raise SSIPCommunicationError(
                "Unknown communication method in address.")
    ...
    def _server_spawn(self, connection_args):
        ...
        if os.path.exists(cmd):
            ...
            if retcode != 0:
                raise SpawnError(
                    "Server refused to autospawn, stating this reason: %s" %
                    (stderr_reply,))
            return server.pid
        else:
    ...
    def set_priority(self, priority):
        ...
        """
        assert priority in (Priority.IMPORTANT, Priority.MESSAGE, Priority.TEXT,
                            Priority.NOTIFICATION, Priority.PROGRESS), priority
