...
def main() -> None:
    ...
    if not os.path.isfile(_file_path):
        ...
        not readtexttools.have_posix_app("text2wave", False)
        and not readtexttools.have_posix_app("flite", False)
        and not bool(readtexttools.get_nt_path("festival/festival.exe"))
    ):
        usage()
        sys.exit(0)
    try:
    ...
    if (
        _read_festival.in_festival_defaults(_sable_rate)
        and _read_festival.in_festival_defaults(_sable_pitch)
        and _read_festival.in_festival_defaults(_sable_speaker)
    ):
        # Pass plain text
        _content = readtexttools.clean_str(_content, False)
        _file_content = _content
        _read_festival.festival_read(
            _file_path,
            _visible,
            _audible,
            _output,
        ...
        _content = readtexttools.local_pronunciation(
            _eval_lang, _content, "festival", "FESTIVAL_TTS_USER_DIRECTORY", False
        )[0]
        _file_content = """<SABLE><SPEAKER NAMEf="{_sable_speaker}">
<RATE SPEED="{_sable_rate}"><PITCH BASE="{_sable_pitch}">
{_content}</PITCH></RATE></SPEAKER></SABLE>"""
