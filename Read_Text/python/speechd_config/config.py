    ...
    def festival_with_freebsoft_utils(self):
        ...
        if not self.festival_socket:
            if not self.festival_connect():
                return False
        self.festival_socket.send(
            bytes("(require 'speech-dispatcher)\n", "ascii"))
...
what is wrong)"""), True):
        ...
        return {
            'spd_say_working': spd_say_working,
            'audio': working_audio,
...
Please make sure your Speech Dispatcher is not running now."""))
        ...
        debugarchive_path = os.path.join(
            tmpdir,
            "speechd-debug-%d-%d-%d.tar.gz" % (date.day, date.month, date.year))
    ...
    def configure_basic_settings(self, type='user'):
        ...
        self.default_output_module = question_with_suggested_answers(
            "Default output module", "espeak-ng", [
                ...
                "swift-generic", "epos-generic", "dtk-generic", "ibmtts",
                "cicero", "kali", "mary-generic", "baratinoo"
            ])

        self.default_language = question(
            "Default language (two/three-letter iso language code like \"en\" or \"cs\", possibly with a region qualification.)",
            "en-US")
        ...
        default_speech_pitch_range = question(
            _("Default speech pitch range (on the scale of -100..100, 0 is default, 50 is higher, -50 is lower)"),
            "0")
        ...
        if type == 'user':
            ...
            configfile = os.path.join(self.test.system_conf_dir(),
                                      "speechd.conf")

        self.options_substitute(
            configfile, {
                ...
                "DefaultPitchRange": default_speech_pitch_range,
            })
        if type == 'user':
...
This is usually not necessary, most applications will start Speech Dispatcher automatically."""),
            ...
            if setup_autostart:
                shutil.copy(
                    os.path.join(buildconfig.SPD_DESKTOP_CONF_PATH,
                                 "speechd.desktop"),
...
def main():
    global dont_ask, use_espeak_synthesis

    options = setup_argparse()
    options = options.parse_args()
    dont_ask = options.dont_ask
    use_espeak_synthesis = options.use_espeak_synthesis
    test = Tests()
    configure = Configure(
        test)  # that should possibly be refactored, test should not be passed
